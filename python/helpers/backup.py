...
class BackupService:
    """
    Core backup and restore service for Sentinel.
    ...
    def __init__(self):
        self.sentinel_version = self._get_sentinel_version()
        self.sentinel_root = files.get_abs_path("")  # Resolved Sentinel root

        # Build base paths map for pattern resolution
        self.base_paths = {
            self.sentinel_root: self.sentinel_root,
        }
    ...
    def get_default_backup_metadata(self) -> Dict[str, Any]:
        ...
        return {
            "backup_name": f"sentinel-backup-{timestamp[:10]}",
            "include_hidden": True,
    ...
    def _get_default_patterns(self) -> str:
        ...
        Only includes Sentinel project directory patterns.
        """
        # Ensure paths don't have double slashes
        agent_root = self.sentinel_root.rstrip('/')
...
"""
    ...
    async def _get_environment_info(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "sentinel_root": files.get_abs_path(""),
    ...
    def _translate_patterns(self, patterns: List[str], backup_metadata: Dict[str, Any]) -> List[str]:
        ...
        Replaces the backed up Sentinel root path with the current Sentinel root path
        ...
        Args:
            ...
            backup_metadata: Backup metadata containing the original sentinel_root

        Returns:
            List of translated patterns for the current system
        """
        # Get the backed up sentinel root path from metadata (with legacy key fallback)
        environment_info = backup_metadata.get("environment_info", {})
        backed_up_agent_root = environment_info.get("sentinel_root") or environment_info.get("agent_zero_root", "")

        # Get current Sentinel root path
        current_agent_root = self.sentinel_root
        ...
        for pattern in patterns:
            # Check if the pattern starts with the backed up sentinel root
    ...
    async def create_backup(
        ...
        backup_name: str = "sentinel-backup"
    ) -> str:
    ...
    def _translate_restore_path(self, archive_path: str, backup_metadata: Dict[str, Any]) -> str:
        ...
        # Check if the archive path starts with the backed up sentinel root
